    ...
    <div className={styles.container}>
      ...
      <div className={styles.chartCard} style={{ marginBottom: '32px' }}>
        ...
        <div className={styles.heatmapGrid}>
          {/*
            Placeholder heat cells until Phase 6 wires this to real message
            volume. Derived from the index rather than Math.random() so the
            grid is stable across renders — random values during render are
            impure and make the mock flicker.
          */}
          {Array.from({ length: 75 }).map((_, i) => {
            const intensity = ((i * 37) % 100) / 100;
            let bg = '#f1f5f9'; // Low
            if (intensity > 0.6) bg = '#d1fae5';
            if (intensity > 0.8) bg = 'var(--status-green)';
            if (intensity > 0.95) bg = '#0d5c46'; // High
